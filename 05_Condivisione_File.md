...
**`/hosthome`** va abilitata esplicitamente con `--hosthome` in [[03_Comandi_Kathara#lstart|lstart]] / [[03_Comandi_Kathara#vstart|vstart]], oppure nelle impostazioni globali. Monta la home dell'utente host direttamente nel device.

> Per controllare il mount di `shared/` su singoli device, si può usare l'opzione `[shared]` in [[04_Struttura_Laboratorio#labconf|lab.conf]].
